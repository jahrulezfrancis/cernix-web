...
export function classifyPlanningJobFailure(error: unknown, attemptNumber: number, maxAttempts: number,
  ...
  if (interrupted) {
    ...
    retryable = RETRYABLE_PLANNING_CODES.has(error.failureCode) || error.failureCode === "qwen_malformed_response";
