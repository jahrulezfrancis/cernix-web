...
import { PlanningWorker, type PlanningWorkerLogger } from "./planning-worker";
import { readPlanningWorkerConfig } from "./planning-worker-config";

const logger: PlanningWorkerLogger = { info(event, fields) { console.log(JSON.stringify({ event, ...fields })); } };

export async function runPlanningWorkerCli(arguments_: readonly string[] = process.argv.slice(2)): Promise<number> {
  ...
  try {
    const workerConfig = readPlanningWorkerConfig(), db = getDatabase();
    const planner = createInvestigationPlanningService();
    const worker = new PlanningWorker(new PlanningJobRepository(db), planner, {
...
if (process.argv[1] && pathToFileURL(process.argv[1]).href === import.meta.url) {
  runPlanningWorkerCli().then((code) => { process.exitCode = code; }, () => { process.exitCode = 1; });
}
