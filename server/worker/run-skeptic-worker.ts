...
import { createInvestigationSkepticService } from "@/server/qwen/skeptic-service-factory";
import { SkepticJobRepository } from "./skeptic-job-repository";
import { SkepticWorker, type SkepticWorkerLogger } from "./skeptic-worker";
import { readSkepticWorkerConfig } from "./skeptic-worker-config";

const logger: SkepticWorkerLogger = { info(event, fields) { console.log(JSON.stringify({ event, ...fields })); } };

export async function runSkepticWorkerCli(arguments_: readonly string[] = process.argv.slice(2)): Promise<number> {
  ...
  try {
    const workerConfig = readSkepticWorkerConfig(), db = getDatabase();
    const skeptic = createInvestigationSkepticService();
    const worker = new SkepticWorker(new SkepticJobRepository(db), skeptic, {
...
if (process.argv[1] && pathToFileURL(process.argv[1]).href === import.meta.url) {
  runSkepticWorkerCli().then((code) => { process.exitCode = code; }, () => { process.exitCode = 1; });
}
