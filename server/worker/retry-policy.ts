...
const RETRYABLE_SNAPSHOT_CODES = new Set<SnapshotFailureCode>([
...
export function classifySnapshotJobFailure(error: unknown, attemptNumber: number, maxAttempts: number,
  ...
  if (interrupted) {
    ...
    retryable = RETRYABLE_SNAPSHOT_CODES.has(error.failureCode);
