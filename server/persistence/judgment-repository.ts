...
async function buildEvidenceSummary(db: Db, investigationId: string): Promise<unknown> {
  const runs = await db.selectFrom("evidence_task_runs").selectAll().where("investigation_id", "=", investigationId).execute();
  const tasks = [];
  for (const run of runs) {
    const candidates = await db.selectFrom("evidence_candidates").selectAll().where("run_id", "=", run.id).execute();
    const candidateSummaries = [];
    for (const candidate of candidates) {
      const excerpts = await db.selectFrom("evidence_excerpts").select(["path", "line_start", "line_end", "excerpt_text"])
    ...
    const gaps = await db.selectFrom("evidence_gaps").select(["gap_key", "description", "impact"]).where("run_id", "=", run.id).execute();
    const counters = await db.selectFrom("counterevidence_items").select(["counter_key", "description", "severity", "related_candidate_key"])
