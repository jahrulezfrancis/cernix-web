    ...
    await sql`
      ...
      create table repository_snapshot_entries (
        ...
        exclusion_reason text constraint repository_snapshot_entries_reason_check check (exclusion_reason is null or exclusion_reason in (
          'tree','submodule','symlink','malformed_git_entry','unsafe_path','generated_directory','dependency_directory','secret_path','unsupported_file_type','lockfile','minified_bundle','source_map','reported_file_too_large','file_count_limit','total_bytes_limit','blob_mismatch','unsupported_encoding','content_size_mismatch','file_too_large','binary_content','invalid_utf8','secret_detected','line_count_limit'
        )),
        manifest_order integer not null constraint repository_snapshot_entries_order_check check (manifest_order >= 0),
        unique(snapshot_id, path), unique(snapshot_id, manifest_order), unique(id, snapshot_id),
      ...
      create table repository_snapshot_files (
        ...
        created_at timestamptz not null,
        constraint repository_snapshot_files_entry_fk foreign key(entry_id, snapshot_id) references repository_snapshot_entries(id, snapshot_id) on delete cascade
      );
