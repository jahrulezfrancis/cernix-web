...
export function advanceSimulation(state: InvestigationSimulationState, force = false): InvestigationSimulationState {
  if ((!state.running && !force) || state.completed) return state;
